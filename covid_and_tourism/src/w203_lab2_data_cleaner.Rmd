...
### The data clean up process divided into three parts by dataset.

## Part 1. [Airbnb Listings Data](http://insideairbnb.com/get-the-data.html)

### ~ Load and Consolidate Raw Airbnb Listings Data ~
```{r load and consolidate raw Airbnb listings data, message=FALSE, warning=FALSE}
...
min_last_scraped <- format(min(listings$last_scraped), '%b %d, %Y')
max_last_scraped <- format(max(listings$last_scraped), '%b %d, %Y')

min_availability_365 <- min(listings$availability_365)
...
3. It appears that the minimum value for `availability_365` is 0. This tells me that some Airbnbs do not have any availability for the next 365 days. The availability variables will need to be explored further.
...
listings_na_values <- listings %>% 
  filter(is.na(id) | is.na(last_scraped) 
         ...
         | is.na(review_scores_value) | is.na(number_of_reviews)
...
min_number_of_reviews <- format(min(listings$number_of_reviews), big.mark=",")
max_number_of_reviews <- format(max(listings$number_of_reviews), big.mark=",")
number_of_reviews_rng <- sprintf("%s - %s",min_number_of_reviews, max_number_of_reviews)
```
...
2. The range of values for `price` and `number_of_reviews` is very large. It is possible these variables will require a log transform for this analysis:
    * `price`: `r price_rng`
    * `number_of_reviews`: `r number_of_reviews_rng`
...
listings_0_365 <- listings %>% filter(availability_60==60)
...
## Part 2. [COVID-19 Vaccinations in the United States, Jurisdiction](https://data.cdc.gov/Vaccinations/COVID-19-Vaccinations-in-the-United-States-Jurisdi/unsk-b7fc)

### ~ Load CDC Vaccine Data ~
```{r Load CDC Vaccine Data, warning=FALSE, message=FALSE}
# Load CDC Vaccine Data.
raw_vaccine <- read_csv("./data/raw/cdc_vaccine_data/COVID-19_Vaccinations_in_the_United_States_Jurisdiction.csv")
...
## Part 3. [COVID-19 US State Policy Database](https://www.openicpsr.org/openicpsr/project/119446/version/V114/view?path=/openicpsr/119446/fcr:versions/V114/COVID-19-US-State-Policy-Database-master&type=folder)

### ~ Load COVID-19 US State Policy Data ~
```{r Load COVID-19 US State Policy Data, warning=FALSE, message=FALSE}
# Load COVID-19 US State Policy Data.
raw_state_policy <- read_excel("./data/raw/covid_state_policy_data/COVID-19 US state policy database 7_14_2021.xlsx", sheet = 1)
...
state_abrv <- read_csv("./data/raw/state_abrv.csv") # Read in State Abbreviation csv.
